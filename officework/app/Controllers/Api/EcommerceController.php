...
class EcommerceController
{
    ...
    private function configData(): array
    {
        ...
        ZoneSchema::ensure();
        $defaultAppName = match ($moduleKey) {
            'medical' => 'City Medical',
            'ecommerce' => 'City E-Commerce',
            default => 'AIMEDIX MEDS Mart',
        };
        ...
        if ($moduleKey === 'medical') {
            $cmsPages[] = [
                ...
                'content' => Settings::moduleGet($moduleKey, 'prescription_policy'),
            ];
        }

        return [
            'app_name' => Settings::moduleGet($moduleKey, 'app_name', $defaultAppName),
    ...
    public function home(): void
    {
        ...
        if ($this->moduleKey() === 'medical' && !$this->hasActiveRequestZone($db)) {
    ...
    public function bestSellingProducts(): void
    {
        ...
        $rows = Database::connection()->query(
            'select products.*, brands.name as brand_name, vendors.shop_name as vendor_name,
                    coalesce(sum(case when orders.id is not null then order_items.quantity else 0 end), 0) as sold_quantity
             from products
             left join brands on brands.id = products.brand_id
             left join vendors on vendors.id = products.vendor_id
    ...
    private function formatProduct(array $row): array
    {
        ...
        $row['medicine_type'] = $row['medicine_type'] ?? 'otc';
        $row['schedule_tag'] = $row['schedule_tag'] ?? '';
        $row['max_qty_per_order'] = $row['max_qty_per_order'] === null ? null : (int) $row['max_qty_per_order'];
        $row['max_qty_per_month'] = $row['max_qty_per_month'] === null ? null : (int) $row['max_qty_per_month'];
        $row['requires_pharmacist_review'] = !empty($row['requires_pharmacist_review']);
        $row['requires_age_confirmation'] = !empty($row['requires_age_confirmation']);
    ...
    private function catalogWhere(): string
    {
        ZoneSchema::ensure();
        if ($this->moduleKey() !== 'medical') {
            return 'products.status = 1 and products.module_key = \'' . $this->moduleKey() . '\' and (products.vendor_id is null or (vendors.module_key = \'' . $this->moduleKey() . '\' and vendors.status = \'approved\' and coalesce(vendors.is_temporarily_closed, 0) = 0))' . ZoneSchema::inlineSql('products');
        }

        \App\Support\MedicalServiceSchema::ensure();
        if (ZoneSchema::requestZoneId() < 1) {
            return "products.status = 1 and products.module_key = 'medical' and 1 = 0";
        }
        return "products.status = 1 and products.provider_visibility = 1 and products.module_key = 'medical' and vendors.module_key = 'medical' and vendors.status = 'approved' and coalesce(vendors.is_temporarily_closed, 0) = 0 and exists (select 1 from medical_providers mp join zones active_product_zone on active_product_zone.id = products.zone_id and active_product_zone.status = 1 where mp.vendor_id = vendors.id and mp.provider_type = 'pharmacy' and mp.status = 'approved')" . ZoneSchema::inlineSql('products');
    }

    private function hasActiveRequestZone(\PDO $db): bool
    {
        $zoneId = ZoneSchema::requestZoneId();
        if ($zoneId < 1) {
            return false;
        }
        $stmt = $db->prepare('select id from zones where id = :id and status = 1 limit 1');
