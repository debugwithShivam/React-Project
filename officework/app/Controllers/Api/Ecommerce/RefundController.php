...
final class RefundController
{
    ...
    public function store(int $orderId): void
    {
        ...
        $duplicate = $db->prepare('select id from refund_requests where order_id = :order_id and coalesce(order_item_id, 0) = :order_item_id and guest_id = :guest_id and status in (\'pending\', \'approved\', \'processing\', \'refund_failed\', \'refunded\') limit 1');
        ...
        $stmt = $db->prepare(
            'insert into refund_requests (order_id, order_item_id, vendor_id, guest_id, customer_id, customer_name, customer_phone, amount, reason, note, status, created_at, updated_at)
             values (:order_id, :order_item_id, :vendor_id, :guest_id, :customer_id, :customer_name, :customer_phone, :amount, :reason, :note, :status, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)'
        );
        $stmt->execute([
            'order_id' => $orderId,
        ...
        NotificationLog::record('admin', null, null, 'Refund requested', 'Refund requested for ' . $order['order_number'], $orderId, $this->moduleKey);
        if ($item && !empty($item['vendor_id'])) {
            NotificationLog::record('vendor', (int) $item['vendor_id'], null, 'Refund requested', $item['product_name'] . ' refund requested in ' . $order['order_number'], $orderId, $this->moduleKey);
        }
