...
final class CustomerController
{
    public function register(): void
    {
        ...
        Security::enforceLoginThrottle('ecommerce-customer-register', $phone);
    ...
    public function login(): void
    {
        ...
        $phone = trim($body['phone'] ?? '');
        $password = (string) ($body['password'] ?? '');
        Security::enforceLoginThrottle('ecommerce-customer-login', $phone);
        if ($phone === '' || $password === '') {
    ...
    public function updateProfile(): void
    {
        ...
        if ($name === '' || strlen($name) > 190) {
            Response::json(['message' => 'Enter a valid name.'], 422);
            return;
        }
        ...
        $update = Database::connection()->prepare('update customers set name=:name,email=:email,updated_at=CURRENT_TIMESTAMP where id=:id');
        $update->execute([
            'id' => (int) $customer['id'],
            'name' => $name,
            'email' => $email,
        ]);
        $fresh = $this->customer((int) $customer['id']) ?: [];
        unset($fresh['password'], $fresh['auth_token']);
        Response::json([
            'message' => 'Profile updated.',
            'data' => $fresh,
            'guest_id' => $this->guestId((int) $customer['id']),
        ]);
    }
    ...
    public function saveAddress(): void
    {
        ...
        $latitude = $this->coordinate($body['latitude'] ?? null, 90);
        $longitude = $this->coordinate($body['longitude'] ?? null, 180);
        $medicalRequest = str_contains((string) ($_SERVER['REQUEST_URI'] ?? ''), '/api/v1/medical/');
        if ($medicalRequest) {
            if ($latitude === null || $longitude === null || ($latitude === 0.0 && $longitude === 0.0)) {
                Response::json(['message' => 'Select the exact delivery location on the map.'], 422);
                return;
            }
            $zone = ZoneSchema::resolve(
                $latitude,
                $longitude,
                trim((string) ($body['pincode'] ?? '')),
                trim((string) ($body['city'] ?? ''))
            );
            if ($zone === null) {
                Response::json(['message' => 'This address is outside our active delivery areas. Choose another location.'], 422);
                return;
            }
        }
        ...
        $stmt = $db->prepare(
            'insert into customer_addresses (customer_id, label, contact_name, contact_phone, address, city, state, pincode, latitude, longitude, is_default, created_at, updated_at)
             values (:customer_id, :label, :contact_name, :contact_phone, :address, :city, :state, :pincode, :latitude, :longitude, :is_default, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)'
        );
        $stmt->execute([
    ...
    private function ensureAddressColumns(\PDO $db): void
    {
        foreach (['latitude', 'longitude'] as $column) {
            try {
                $db->query('select ' . $column . ' from customer_addresses limit 1');
            } catch (\Throwable) {
                $type = $db->getAttribute(\PDO::ATTR_DRIVER_NAME) === 'mysql' ? 'decimal(10,7) null' : 'real';
                $db->exec('alter table customer_addresses add column ' . $column . ' ' . $type);
            }
        }
    }
    ...
    private function coordinate(mixed $value, float $limit): ?float
    {
        ...
        return abs($coordinate) <= $limit && $coordinate !== 0.0 ? $coordinate : null;
    }
}
