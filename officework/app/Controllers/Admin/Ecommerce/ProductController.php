...
{
    public function __construct(private readonly string $moduleKey = 'ecommerce', private readonly string $basePath = '/admin/ecommerce/products')
    {
    }
    ...
    public function store(): void
    {
        ...
        $stmt = Database::connection()->prepare(
            'insert into products (module_key, zone_id, vendor_id, brand_id, category_id, subcategory_id, name, slug, description, unit, price, discount_price, stock, sku, thumbnail, tax_percent, shipping_cost, barcode, seo_title, seo_description, attributes_json, colors_json, is_digital, digital_file_url, is_flash_deal, flash_deal_ends_at, is_clearance, freshness_note, expiry_date, shelf_life, warranty_note, return_policy, medicine_type, schedule_tag, max_qty_per_order, max_qty_per_month, requires_pharmacist_review, requires_age_confirmation, status, is_featured, created_at, updated_at)
             values (:module_key, :zone_id, :vendor_id, :brand_id, :category_id, :subcategory_id, :name, :slug, :description, :unit, :price, :discount_price, :stock, :sku, :thumbnail, :tax_percent, :shipping_cost, :barcode, :seo_title, :seo_description, :attributes_json, :colors_json, :is_digital, :digital_file_url, :is_flash_deal, :flash_deal_ends_at, :is_clearance, :freshness_note, :expiry_date, :shelf_life, :warranty_note, :return_policy, :medicine_type, :schedule_tag, :max_qty_per_order, :max_qty_per_month, :requires_pharmacist_review, :requires_age_confirmation, :status, :is_featured, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)'
        );
        $stmt->execute([
            ...
            'is_digital' => $this->moduleKey === 'medical' ? 0 : (isset($_POST['is_digital']) ? 1 : 0),
            'digital_file_url' => $this->moduleKey === 'medical' ? '' : trim($_POST['digital_file_url'] ?? ''),
            ...
            'medicine_type' => $this->medicineType($_POST['medicine_type'] ?? 'otc'),
            'schedule_tag' => trim($_POST['schedule_tag'] ?? ''),
            'max_qty_per_order' => ($_POST['max_qty_per_order'] ?? '') === '' ? null : max(1, (int) $_POST['max_qty_per_order']),
            'max_qty_per_month' => ($_POST['max_qty_per_month'] ?? '') === '' ? null : max(1, (int) $_POST['max_qty_per_month']),
            'requires_pharmacist_review' => isset($_POST['requires_pharmacist_review']) ? 1 : 0,
            'requires_age_confirmation' => isset($_POST['requires_age_confirmation']) ? 1 : 0,
    ...
    private function sku(string $name, string $input, string $current = ''): string
    {
        ...
        $modulePrefix = $this->moduleKey === 'medical' ? 'MED' : 'PRD';
        return $modulePrefix . '-' . $prefix . '-' . date('ymdHis') . '-' . random_int(100, 999);
    }

    private function medicineType(string $value): string
    {
        return in_array($value, ['otc', 'prescription_required', 'restricted', 'blocked_online'], true)
            ? $value
            : 'otc';
    }
